		...
		if ( props != null ) {
			for (String prop: props) {
				...
				if (prop.equals("*")) {
					...
					// equals variable
					String[] p = prop.split(":=");
					ok = validateEqualsVar(values, new String[] {p[0]}, new String[] {p[1]}) && ok;
				} else if (prop.contains("::")) {
					// number check
					String[] p = prop.split("::");
					double min = Double.parseDouble(p[1]);
					double max = Double.parseDouble(p[2]);
					int precision = (p[1].contains("."))?p[1].substring( p[1].indexOf(".") ).length():0;
					ok = validateNumber(values, min, max, precision, p[0]) && ok;
				} else if ( prop.contains( ":" ) ) {
						int i = prop.indexOf(':');
						ok = validateJs( prop.substring(0,i), prop.substring(i+1), values ) && ok;
				} else if (prop.contains("==")) {
					// equals value
					String[] p = prop.split("==");
					ok = validateEquals(values, new String[] {p[0]}, new String[] {p[1]}) && ok;
				} else {
					// null or empty
...
//					System.err.println(prop + " : " + val);
	...
	public boolean validateEqualsVar(ExpData values, String[] props, String[] vals)  {
		if (logger.isDebugEnabled()) {
	...
	public boolean validateNumber(ExpData values, double min, double max, int precision, String... props)  {
		if (logger.isDebugEnabled()) {
...
//					L.println("validate " + d + " min " + min + " max" + max); 
					if ( d < min ) {
						ok = false;
						valid = false;
						put(prop, tooSmall);						
					...
					int numprecision = (num.contains("."))?num.substring( num.indexOf(".") ).length():0;
